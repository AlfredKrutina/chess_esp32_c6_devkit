...
- **`downloads.html`** — veřejná stránka ke stažení APK/DMG + texty kolem CzechMate. **V2** (Hall, komerce) vs **V1** (reed, video, fw **1.8.0**) je vysvětlené přímo na stránce; dotazníky a předobjednávka = **V2**. Sekce **Aplikace**: karty otevírají modal — obsah v `landing/landing.js` (`FEATURE_PAGES`). Tlačítka APK/DMG mají fallback na release; `landing.js` dotáhne z GitHub API `releases/latest` přímé `browser_download_url`.
