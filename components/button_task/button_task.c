...
static bool button_long_press_sent[CHESS_BUTTON_COUNT] = {
    false}; // Udalost dlouheho stisku odeslana
...
static const char *button_names[] = {
    ...
    "White Promotion Bishop",
    "White Promotion Knight", // 0-3: White promotion buttons
    "Black Promotion Queen",
    "Black Promotion Rook",
    "Black Promotion Bishop",
    "Black Promotion Knight", // 4-7: Black promotion buttons
    "Reset"                   // 8: Reset button
};

// Button LED indices - REMOVED
// LED indications are now controlled by game_task via
// game_check_promotion_needed() LED mapping:
...
void button_scan_all(void) {
  ...
  if (simulation_mode) {
    ...
    if (current_time - last_simulation_time > 5000) {
      ...
      current_simulation_button =
          (current_simulation_button + 1) % CHESS_BUTTON_COUNT;
    ...
    if (button_simulate_release_time > 0 &&
        current_time >= button_simulate_release_time) {
...
void button_simulate_press(uint8_t button_id) {
  ...
  ESP_LOGI(TAG, "Simulating button press: %s (ID: %d)", button_names[button_id],
           button_id);
...
void button_simulate_release(uint8_t button_id) {
  ...
  uint32_t press_duration =
      esp_timer_get_time() / 1000 - button_press_time[button_id];
  ...
  if (press_duration >= BUTTON_LONG_PRESS_MS &&
      !button_long_press_sent[button_id]) {
...
void button_handle_press(uint8_t button_id) {
  ...
  ESP_LOGI(TAG, "Button pressed: %s (ID: %d)", button_names[button_id],
           button_id);
...
void button_check_double_press(uint8_t button_id) {
  ...
  uint32_t time_since_last_release =
      current_time - button_release_time[button_id];

  if (time_since_last_release <= BUTTON_DOUBLE_PRESS_MS) {
    button_press_count[button_id]++;

    if (button_press_count[button_id] == 2) {
      ESP_LOGI(TAG, "Double press detected: %s (ID: %d)",
               button_names[button_id], button_id);
...
void button_send_event(uint8_t button_id, button_event_type_t event_type,
                       uint32_t duration) {
  if (button_id >= CHESS_BUTTON_COUNT)
    return;

  // Create button event
  button_event_t event = {.type = event_type,
                          .button_id = button_id,
