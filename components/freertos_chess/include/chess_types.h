 ...
 *
 * @details
 * Tento soubor definuje vsechny zakladni datove typy pouzivane v sachovem
 * systemu:
...
typedef enum {
  ...
  MOVE_ERROR_DESTINATION_OCCUPIED =
      14,                          ///< Cilove pole je obsazeno vlastni figurkou
...
typedef struct {
  ...
  promotion_choice_t
      promotion_piece; ///< Figurka pro promoci (pokud je to promoce)
...
typedef enum {
  ...
  LED_CMD_BUTTON_PROMOTION_AVAILABLE =
      27, ///< Nastav promocni tlacitko jako dostupne
  LED_CMD_BUTTON_PROMOTION_UNAVAILABLE =
      28,                          ///< Nastav promocni tlacitko jako nedostupne
  ...
  LED_CMD_SHOW_LEGAL_MOVES =
      39, ///< Zobraz vsechny legalni tahy pro typ figurky

  // Prikazy Enhanced Castling systemu
  LED_CMD_CASTLING_GUIDANCE =
      40,                      ///< Zobraz navod pro rosadu (pozice krale/veze)
...
typedef struct {
  button_event_type_t
      type; ///< Typ udalosti (press, release, long press, double press)
